"""
mahnoor-temp-ingest-hvac
Lambda triggered by SQS. Parses HVAC messages and saves them to DynamoDB.
After saving, invokes the processor Lambda via DynamoDB Streams (automatic).
"""
...
def handler(event, context):
    ...
    Returns partial failure info so SQS retries only failed messages.
    """
    batch_item_failures = []
...
def save_to_dynamodb(payload: dict):
    """Parse and persist a single HVAC reading."""
